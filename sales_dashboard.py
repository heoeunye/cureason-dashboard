...
SHEET_ID = "1EF9_w3KB4ncTgbGruWt6KH0IzI7FazvIsNQDkN_Agh8"
...
def get_sheet():
    ...
    try:
        ...
        ws = sh.add_worksheet("sales", rows=10, cols=2)
...
def parse_sales_excel(file_bytes):
    ...
    if not target: target = [wb.active.title]
    ...
    for sname in target:
        ...
        if ph_row is None:
            for i, row in enumerate(rows):
                if sum(1 for c in row if c and str(c).strip()=="날짜") >= 2:
                    ph_row = i; break
        if ph_row is None:
            log.append(f"  [{sname}] ❌ 헤더 못 찾음"); continue

        hdr      = rows[ph_row]
        dcols    = [j for j,c in enumerate(hdr) if c and str(c).strip()=="날짜"]
        date_col = dcols[-1] if dcols else None
        if date_col is None:
            log.append(f"  [{sname}] ❌ 날짜 컬럼 없음"); continue
        ...
        for j in range(date_col+1, len(hdr)):
            ...
            if h and str(h).strip() not in ("비고","None",""):
                ...
                if mapped in PLATFORMS: pcols.append((j, mapped))

        cnt = 0
        for row in rows[ph_row+1:]:
            if not row or not row[date_col]: continue
            d = to_date(row[date_col])
            if not d: continue
            if expected_month and d.month == expected_month and d.year != date.today().year:
                try: d = d.replace(year=date.today().year)
                except: pass
        ...
        for i, row in enumerate(rows):
            ...
            if "주문형태" in strs and "주문일" in strs:
                for row2 in rows[i+1:]:
                    if not row2 or not row2[0]: continue
                    try:
...
with tab_sum:
    ...
    for p in PLATFORMS:
        ...
        r[0].write(p); r[1].write(fmt_krw(cp)); r[2].write(fmt_krw(pp) if pp else "—")
    ...
    if uploaded:
        with st.spinner("전체 시트 파싱 중..."):
            try:
                ...
                import traceback
                st.error(f"오류: {e}"); st.code(traceback.format_exc())
        ...
        if not parsed_sales:
            ...
            for mk in sorted(parsed_sales.keys()):
                md = parsed_sales[mk]
                st.markdown(f"**{mk} — {fmt_krw(month_total(md))}**")
                active = [(p, platform_total(md,p)) for p in PLATFORMS if platform_total(md,p)>0]
                cols = st.columns(4)
                for i,(p,amt) in enumerate(active): cols[i%4].write(f"{p}: {fmt_krw(amt)}")
                st.markdown("---")
            if parsed_orders: st.markdown(f"주문 상세 {len(parsed_orders)}건")
            ...
            if st.button("📥 가져오기 확정", type="primary", use_container_width=True):
                with st.spinner("구글 시트에 저장 중..."):
                    for mk, md in parsed_sales.items():
                        if mk not in data or overwrite: data[mk] = md
                        else:
                            for platform, days in md.items():
                                data[mk].setdefault(platform, {}).update(days)
                    if parsed_orders:
                        if overwrite: data["orders"] = parsed_orders
                        else:
